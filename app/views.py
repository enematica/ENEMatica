...
class SimuladoList(ListView):
    ...
    def get_queryset(self):
        ...
        ano = self.request.GET.get('anos2')
        dificuldade = self.request.GET.get('dificuldades2')
        quant_questoes = self.request.GET.get('quant_questoes2')
        tempo = self.request.GET.get('tempo2')

        # Continuação
        if conteudo != None or ano != None or dificuldade != None or quant_questoes != None or tempo != None:
            if len(conteudo) > 0 or len(ano) > 0 or len(dificuldade) > 0 or len(quant_questoes) > 0 or len(tempo) > 0:
                conteudos = conteudo.split(',')
                anos = ano.split(',')
                dificuldades = dificuldade.split(',')
                quant_questoes = quant_questoes.split(',')[0]
                tempo = tempo.split(',')
                print(quant_questoes,"sdfghjkl")
                # Caso não seja arrumado a seleção mutua:
                if len(tempo) != 1:
                    tempo = tempo[0]
                print(quant_questoes,"sfa")
                print(conteudos)# =+=+=+=+=+=+=+=+=+=+=
                print(anos)# =+=+=+=+=+=+=+=+=+=+=
                print(dificuldades)# =+=+=+=+=+=+=+=+=+=+=
                querysets = []
...
class SimuladoGerado(ListView):
    #
    model = Questoe
    template_name = 'simulado_gerado.html'
    paginate_by = SimuladoList.quant_questoes
